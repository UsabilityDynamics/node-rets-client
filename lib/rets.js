  ...
  request: {
    value: function request(url) {
      ...
      if (options.agent.pass) {
        ...
        var session_id_to_calculate_with = "";
      ...
      req.setopt('URL', url);
      req.setopt('COOKIEJAR', ''); // Cookies are kinda important for sessions
  ...
  sendRequest: {
    ...
    value: function sendRequest( action, args, callback, streamCallback ) {
      ...
      request.on('end', function() {
          ...
          if (request.getinfo('RESPONSE_CODE') !== 200) {
            RETS.debug( 'Error: we received an http - likely an HTML response to follow', body);
            return callback( { code: request.getinfo('RESPONSE_CODE'), request: request }, null)
          }
    ...
    value: function _handleBody( body, callback ) {
      ...
      this.utility.parseString( body, function xml_parsed( error, data ) {
        ...
        try {
          ...
          if(data.RETS[ 'COLUMNS' ]) {
            var count = data.RETS.COUNT[0];
            var delimiter = data.RETS.DELIMITER[0];
            var columns = data.RETS.COLUMNS[0];
            var res = data.RETS.DATA[0];
            ...
            parser.on('finish', function(){
              var _parse = require('csv-parse');
              var _parser = _parse({delimiter: "\t"});
              parsed.data = [];

              _parser.on('readable', function(){
                while(record = _parser.read()){
                  var rec = {};
                  for(var i = 0; i<headers.length; i++) {
                    rec[headers[i]] = record[i];
                  }
                  parsed.data.push(rec);
                }
              });
              // Catch any error
              _parser.on('error', function(err){
                callback(err, null);
              });
              // When we are done, test that the parsed output matched what expected
              _parser.on('finish', function(){
