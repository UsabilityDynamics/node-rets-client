...
function RETS( settings, callback ) {
  ...
  this.set( 'settings', this.utility.defaults( settings, {
    ...
    cookie: '/tmp/'+(Math.floor(((Math.random() * 10) + 1) * 1337))
  }));
  ...
  */
  ...
  request.on('data', function(chunk) {
    /** For our auth exchange - we'll buffer responses rather than stream */
    body += chunk;
    return chunk.length;
  });

  request.on('header', function(chunk) {
    /** Let's handle our own headers; this still needs some work */
...
Object.defineProperties( RETS.prototype, {
  ...
  request: {
    value: function request(url) {
      ...
      if (options.agent.pass) {
        ...
        RETS.debug(this.get('authoization.session'));
      ...
      req.setopt('URL', url);
      req.setopt('COOKIEJAR', this.get( 'settings.cookie' )); // Cookies are kinda important for sessions
  ...
  sendRequest: {
    ...
    value: function sendRequest( action, args, callback, streamCallback ) {
      ...
      /** Since node-curl doesn't use streams we'll setup a basic one first */
      var _stream = function(options) {
        if(!(this instanceof _stream)) return new _stream(options);
        stream.Transform.call(this, options);
      };
      util.inherits(_stream, stream.Transform);

      _stream.prototype._transform = function (chunk, enc, _callback) {
        var $this = this;
        // var _chunks = chunk.toString().split('\n');
        // _chunks.forEach(function(_chunk){ $this.push(chunk) })
        this.push(chunk);
        _callback();
      };

      _stream.prototype._flush = function readBytes(_callback) {
        _callback();
      };

      /** Now we need a real transformer to handle what's coming out of xmlstream */
      var _real_stream = function(options) {
        if(!(this instanceof _real_stream)) return new _real_stream(options);
        stream.Transform.call(this, options);
      };
      util.inherits(_real_stream, stream.Transform);

      _real_stream.prototype._transform = function (chunk, enc, _callback) {
        var $this = this;
        var _chunks = chunk.toString().split('\n');
        _chunks.forEach(function(_chunk){ $this.push(_chunk) })
        // this.push(chunk);
        // this.push(chunk);
        _callback();
      };

      _real_stream.prototype._flush = function readBytes(_callback) {
        _callback();
      };

      var _ts = new _stream();
      var _rs = new _real_stream();
      var xml = new this.utility.XmlStream(_ts);
      streamCallback(_rs);

      xml.preserve('COLUMNS',true);
      xml.on('endElement: COLUMNS', function(row){
        // _rs.write('Row');
        _rs.write(row.$children.toString());
