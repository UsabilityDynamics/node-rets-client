...
function RETS( settings, callback ) {
  ...
  this.set( 'settings', this.utility.defaults( settings, {
    ...
    cookie: '/tmp/'+(Math.floor(((Math.random() * 10) + 1) * 1337))+'.txt'
  }));
  ...
  this.utility.digest({
    ...
    cookie: this.get('settings.cookie'),
  ...
  _requestHeaders: {
    ...
    value: function _requestHeaders() {
      ...
      if( this.get( 'settings.agent.user' ) && this.get( 'settings.agent.password' ) ) {
        ...
        var session = this.get('authorization.session');
        if (session !== '') {
      ...
      if( this.get( 'authorization.cookies' ) ) {
        ...
        console.log(this.get( 'authorization.cookies' ));
        ...
        this.set( 'settings.headers.Set-Cookie', _cookies );
      }
