...
function RETS( settings, callback ) {
  ...
  */
  ...
  RETS.debug(options.port)
  if (options.port === '443') {
    request.setopt('USE_SSL', true);
    request.setopt('SSL_VERIFYPEER', false);
  }
  // if (options.agent.pass) request.setopt('HEADER','')
  request.setopt('HTTPHEADER', 'RETS-Version: '+this.get('authorization.version'));
  RETS.debug(this.get( 'settings.agent' ));
