...
var HTTPDigest = function() {
	...
	HTTPDigest.prototype.request = function( options, callback ) {
		...
		request.on('header', function(chunk) {
			...
			console.log('Header Received:',key,val);
		...
		request.on('end', function() {
				...
				// console.log( 'Error: we received an http error - likely an HTML response to follow', request.getinfo('RESPONSE_CODE'), responseHeaders, body);
